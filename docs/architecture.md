...
`fetch()` is Promise-based and same-origin only. It supports `GET`, `HEAD`, `OPTIONS`, `POST`, `PUT`, `PATCH`, and `DELETE`, string request bodies, `text()`/`json()` response methods, and plain-object headers or the implemented `Headers` subset. The page-side `Headers` class normalizes field names, supports common mutation and iteration methods, and is serialized into the Python request bridge. Response headers remain unavailable. CORS, redirects, streams, `FormData`, `AbortController`, and `Request` are unsupported. The ES-module loader supports static imports plus basic named/default/namespace exports and imports, caches each URL per navigation, and handles simple cycles. Dynamic imports, re-exports, top-level `await`, and live bindings remain unsupported. `XMLHttpRequest` is unavailable.
