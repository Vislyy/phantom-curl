...
`Element.click()`, `Element.type()`, and `Element.dispatch_event()` execute registered DOM event listeners synchronously. Their handlers can mutate the DOM immediately and can queue `fetch()` calls, Promise jobs, or timers. Each Python interaction automatically drains fetches, Promise jobs, and zero-delay timers before returning. Timers scheduled for the future still require `Page.run_event_loop(timeout)`. Linkedom dispatches events but does not implement browser default actions such as navigating an anchor or submitting a form. A script element inserted by an event handler is not automatically fetched and executed after the interaction.
